...
.setLabel(`Skip`),
      ...
      embed.setDescription(`[${track.title}](${track.uri}) \`${track.isStream ? `LIVE STREAM` : format(track.duration)}\`
Requested by <@${track.requester.id}>`)
  ...
  .on("queueEnd", async (player) => {
    ...
    .setColor(ee.color)
    .setAuthor(`| Queue More Songs/Enable AutoPlay!`,client.user.displayAvatarURL(), config.links.opmusicvote) 
                                                  
    try{
      client.channels.cache.get(player.textChannel).send({
      content: `Had fun playing? if yes join Support Server for us 
https://discord.gg/ATccRnaBQu`, embeds: [embed]})
    } catch {/* */}
    return ;
  });
};     